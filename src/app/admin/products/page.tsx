...
type ProductsPageProps = {
  searchParams?: Promise<{ q?: string; featuredError?: string }>;
};
    ...
    <div className="space-y-6">
      ...
      {featuredError === "limit" ? (
        <div className="border border-red-600 bg-red-50 px-4 py-3 text-sm text-red-700">
          You already have {MAX_FEATURED} featured products. Unfeature one first, then feature another.
        </div>
      ) : null}
                ...
                <tr key={product.id} className={product.featured ? "bg-emerald-50/40" : undefined}>
                  ...
                  <td className="px-4 py-4">
                    <form action={toggleFeaturedProduct}>
                      ...
                      >
                        {product.featured ? `FEATURED #${product.homepageOrder ?? ""}` : "FEATURE"}
                      </button>
                    </form>
                  </td>
                  <td className="px-4 py-4">
